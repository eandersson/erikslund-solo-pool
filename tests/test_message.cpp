...
TEST_CASE("set_difficulty uses canonical JSON number formatting") {
    // Golden strings pin the decimal point on integer-valued difficulties, which Glaze omits.
    struct Golden {
        double difficulty;
        const char* number;
    };
    ...
    // Canonical e-notation uses lowercase 'e' with no plus sign or leading exponent zeros.
...
TEST_CASE("suggest_difficulty leaves the value unset for non-numeric input") {
    ...
    REQUIRE(r.has_value());
    CHECK_FALSE(r->suggested_difficulty.has_value());
    // Whole-string parsing rejects trailing garbage instead of silently reading the numeric prefix.
