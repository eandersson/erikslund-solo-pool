...
TEST_CASE("SetExtranoncePrefix has a pinned byte vector") {
...
TEST_CASE("Channel request and job frames match pinned wire vectors") {
    U256 target{};
...
TEST_CASE("Pool-emitted frames match pinned wire vectors") {
    const U256 target = increasing_u256();
...
TEST_CASE("NewExtendedMiningJob matches its pinned wire vector") {
