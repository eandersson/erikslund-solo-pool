...
Hash math mirrors tools/miner/miner.py (genesis-verified). stdlib only.
...
class Client:
    def __init__(self, host: str, port: int, address: str, pipeline: int,
        ...
        self.auth_ok = False
        self._mid = self._tail = None  # midstate + 12-byte header tail for the job
        self._net_target = 0
        self._nonce = 0
        self._id = 10
    ...
    def _next_above_target_nonce(self) -> int:
        # Find a nonce whose hash is above the network target (not a real block).
        # ~2 iterations on regtest; happens before the submit goes on the wire.
        while True:
            n = self._nonce
...
async def run_local(args, n_conn: int, barrier=None) -> dict:
    ...
    if barrier is not None:  # rendezvous so all worker processes measure the same window
        try:
...
def _worker(args, n_conn, barrier, q):
    ...
    if len(res["lats"]) > 200000:  # cap per-worker samples sent over the queue
...
def main() -> int:
    ...
    mp.set_start_method("fork", force=True)  # Linux container; inherit the barrier cleanly
    ...
    for i in range(w):
        ...
        p.start()
        procs.append(p)
    results = [q.get() for _ in range(w)]  # drain before join (avoids the Queue deadlock)
    for p in procs:
        p.join()
