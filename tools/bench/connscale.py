    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                continue
            if message.get("id") == 2:  # authorize response -> session fully established
                break
        ESTABLISHED += 1
        await stop.wait()  # hold the connection open for the measurement window
    except Exception:
        FAILED += 1
    finally:
        try:
...
async def main() -> int:
    ...
    for i in range(0, args.connections, args.batch):
        ...
        await asyncio.sleep(0.15)  # let each batch land within the listen backlog
    ...
    await asyncio.sleep(args.hold)  # hold open while the harness samples RSS
    stop.set()
