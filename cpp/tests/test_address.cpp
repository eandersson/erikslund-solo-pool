...
TEST_CASE("mainnet P2PKH -> scriptPubKey") {
    CHECK(to_hex(address_to_script("1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa", Network::Mainnet)) ==
          "76a91462e907b15cbf27d5425399ebf6f0fb50ebb88f1888ac");
}

TEST_CASE("mainnet P2WPKH -> scriptPubKey") {
    CHECK(to_hex(address_to_script("bc1qw508d6qejxtdg4y5r3zarvary0c5xw7kv8f3t4",
                                   Network::Mainnet)) ==
          "0014751e76e8199196d454941c45d1b3a323f1433bd6");
}

TEST_CASE("mainnet P2WSH -> scriptPubKey") {
    CHECK(to_hex(address_to_script(
...
TEST_CASE("P2SH round-trips through the script encoder") {
    ...
    CHECK(to_hex(address_to_script(addr, Network::Mainnet)) ==
...
TEST_CASE("regtest P2PKH path (testnet prefix 0x6f)") {
    ...
    append(payload, hash160);
    const std::string addr = base58check_encode(payload);
    CHECK(to_hex(address_to_script(addr, Network::Regtest)) ==
          "76a91462e907b15cbf27d5425399ebf6f0fb50ebb88f1888ac");
}
...
TEST_CASE("wrong network is rejected") {
    ...
    CHECK_THROWS_AS(
        address_to_script("bc1qw508d6qejxtdg4y5r3zarvary0c5xw7kv8f3t4", Network::Regtest),
    ...
    CHECK_THROWS_AS(address_to_script("1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa", Network::Regtest),
                    std::invalid_argument);
    // Pure garbage.
    CHECK_THROWS_AS(address_to_script("not-an-address", Network::Mainnet), std::invalid_argument);
}
...
TEST_CASE("mainnet P2TR (v1 taproot) -> OP_1 push32 scriptPubKey") {
    ...
    CHECK(to_hex(address_to_script(addr, Network::Mainnet)) == "5120" + to_hex(program));
}
...
TEST_CASE("testnet/signet share the 'tb' bech32 hrp") {
    ...
    CHECK(to_hex(address_to_script(addr, Network::Testnet)) == "0014" + to_hex(program));
    CHECK(to_hex(address_to_script(addr, Network::Signet)) == "0014" + to_hex(program));
...
TEST_CASE("regtest bech32 (bcrt) P2WPKH decodes") {
    ...
    CHECK(to_hex(address_to_script(addr, Network::Regtest)) == "0014" + to_hex(program));
}
...
TEST_CASE("testnet and signet: every address type (shared tb hrp + testnet base58 versions)") {
    ...
    for (const Network net : {Network::Testnet, Network::Signet}) {
        ...
        append(p2pkh, h160);
        CHECK(to_hex(address_to_script(base58check_encode(p2pkh), net)) ==
              "76a914" + to_hex(h160) + "88ac");

        Bytes p2sh{0xc4}; // testnet/signet P2SH version
        append(p2sh, h160);
        CHECK(to_hex(address_to_script(base58check_encode(p2sh), net)) ==
              "a914" + to_hex(h160) + "87");

        CHECK(to_hex(address_to_script(segwit_address_encode("tb", 0, h160), net)) == // P2WPKH
              "0014" + to_hex(h160));
        CHECK(to_hex(address_to_script(segwit_address_encode("tb", 1, prog32), net)) == // P2TR
              "5120" + to_hex(prog32));
    }
}
...
TEST_CASE("testnet4 chain name resolves to testnet address rules end to end") {
    ...
    CHECK(to_hex(address_to_script(segwit_address_encode("tb", 0, h160), *net)) ==
          "0014" + to_hex(h160));
    // Mainnet and regtest forms of the same program are rejected on testnet4.
    CHECK_THROWS_AS(address_to_script(segwit_address_encode("bc", 0, h160), *net),
                    std::invalid_argument);
    CHECK_THROWS_AS(address_to_script(segwit_address_encode("bcrt", 0, h160), *net),
                    std::invalid_argument);
}
